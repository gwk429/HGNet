...
class ThreeDMatchTrainVal(data.Dataset):
    def __init__(self,
        ...
        OVERLAP_RATIO = 0.3
        DATA_FILES = {
            'train': 'F:\\gwk\\PG-Net_new\\PG-Net-master\\misc\\split/train_3dmatch.txt',
            'val': 'F:\\gwk\\PG-Net_new\\PG-Net-master\\misc\\split/val_3dmatch.txt',
    ...
    def __getitem__(self, index):
        ...
        src_pcd.estimate_normals(search_param=o3d.geometry.KDTreeSearchParamKNN(knn=33))
        tgt_pcd.estimate_normals(search_param=o3d.geometry.KDTreeSearchParamKNN(knn=33))
        src_normal = np.array(src_pcd.normals)  # [1000,3]
        ...
        labels = (distance < self.inlier_threshold).astype(int)
        ...
        return corr_pos.astype(np.float32), \
            ...
            #src_features.astype(np.float32), \
        #tgt_features.astype(np.float32),
...
if __name__ == "__main__":
    dset = ThreeDMatchTrainVal(root='/media/lixiaohan/下载/huohu/PointDSC-master/data/3DMatch',
                               split='train',
                               descriptor='fcgf',
                               num_node='all',
                               use_mutual=True,
