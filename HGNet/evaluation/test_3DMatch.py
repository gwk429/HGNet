...
from models.Space_Normal import space_normal, compute_angle, draw_registration_result
...
def eval_3DMatch_scene(model, scene, scene_ind, dloader, config, use_icp):
    ...
    with torch.no_grad():
        for i in tqdm(range(num_pair)):
            ...
            #################################
            data_timer.tic()
            corr, src_keypts, tgt_keypts, src_normal, tgt_normal, gt_trans, gt_labels = next(dloader_iter)
...
def eval_3DMatch(model, config, use_icp):
    ...
    for scene_ind, scene in enumerate(scene_list):
        ...
        all_stats[scene] = scene_stats  #[506, 12]
    ...
    with open('output.txt', 'a') as f:
        print(allpair_average, file=f)
        print(correct_pair_average, file=f)
    # logging.info(f"*" * 40)
...
if __name__ == '__main__':
    ...
    config_path = f'F:/HG-Net-master/snapshot/{args.chosen_snapshot}/config.json'
    #print(config_path)
    ...
    for i in range(1, 51):
        miss = model.load_state_dict(
            torch.load(f'/HG-Net-master/snapshot/{args.chosen_snapshot}/models/model_{i}.pkl'),
             strict=False)  #best
        print(miss)
        print(i)
        with open('output.txt', 'a') as f:
