...
def eval_KITTI(model, config, use_icp):
    dset = KITTIDataset(root='/media/lixiaohan/下载/huohu/PointDSC-master/data/KITTI',
                        split='test',
...
if __name__ == '__main__':
    ...
    if args.use_icp:
        log_filename = f'/HG-Net-master/snapshot/{args.chosen_snapshot}-{args.solver}-ICP.log'
    else:
        log_filename = f'/HG-Net-master/snapshot/{args.chosen_snapshot}-{args.solver}.log'
    ...
    config_path = f'/HG-Net-master/snapshot/{args.chosen_snapshot}/config.json'
    ...
    for i in range(50, 101):
        miss = model.load_state_dict(torch.load(
            f'/HG-Net-master/snapshot/{args.chosen_snapshot}/models/model_{i}.pkl'),
                                     strict=False)
        print(miss)
        model.eval()
